...
class FileManager {
  ...
  private:
    ...
    bool createBackup(const char* nomFichier) {
      ...
      File fichier = sd->open(nomFichier, FILE_READ); // Open the original file

      if (fichier) {
        File fichierBackup = sd->open(nomFichierBackup, FILE_WRITE); // Open the backup file

        if (fichierBackup) {
          while (fichier.available()) {
            fichierBackup.write(fichier.read()); // Copy data from original to backup
          }

          fichierBackup.close(); // Close the backup file

          fichier.close(); // Close the original file

          setLastMessage(F("Création du backup OK"));
          return true;
        } else {
          setLastMessage(F("Erreur lors de l'ouverture du fichier de backup"));
        }

        fichier.close(); // Close the original file
      } else {
        setLastMessage(F("Erreur lors de l'ouverture du fichier d'origine"));
      }
